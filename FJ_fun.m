function [Spectrum, freq, trial_v] = FJ_fun(fmin,fmax,V_min,V_max,dv,y,Fs,cha,r,Spec_Whitening,Trace_norm)
...
Bessel1 = besselj(0,kr);                        % Bessel function of first kind 
...
for i = 1:length(r_new)-1
    ...
    Spectrum = Spectrum + (R_stack(:,i).*Bessel1(:,:,i).*r_new(i) + R_stack(:,i+1).*Bessel1(:,:,i+1).*r_new(i+1))*0.5*dr; 
